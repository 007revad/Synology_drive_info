...
#----------------------------------------------------------
# Show Synology smart health and attributes
#
...
if [[ -d "/var/packages/drive_info/var" ]]; then
    logpath="/var/packages/drive_info/var/"
else
    logpath="/var/packages/drive_info/etc/"
fi
...
def format_smart_line():
    ...
    COLOR_IDS = {1, 5, 7, 10, 187, 188, 196, 197, 198, 199, 200}
...
for drive in "${nvmes[@]}"; do
    ...
    if echo "$m2_card" | grep -q 'Not M.2 adapter card'; then
        m2_card=""
        nvmes_sorted+=("${drive_number:?},${drive:?}")
    else
        nvmes_sorted+=("${drive_number:?} (${m2_card}),${drive:?}")
    fi
done
